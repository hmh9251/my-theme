...
一个基于 [Night Owl Light](https://github.com/sdras/night-owl-vscode-theme)(作者 Sarah Drasner)的浅色 VS Code 主题。在原样保留 Night Owl Light 语法配色的基础上,叠加了**经典深灰活动栏 + 蓝色状态栏**,以及**圆角 / 活动-非活动高度差标签**等美化。
...
- **字符串**:舒适绿 `#22863A`
- **对象键名**:蓝灰 `#0B7CAD`(v0.9.1+),通过 `constant.other.object.key.*` 精确匹配,与字符串明显区分
...
```powershell
code --install-extension dawn-owl-1.0.0.vsix --force
```
...
VS Code 只开放标签颜色,不开放圆角与每标签高度。要实现需向 workbench 注入 CSS:
...
- `1.0.0` 字符串恢复为绿 `#22863A`,与对象键名蓝灰 `#0B7CAD` 形成清晰区分
